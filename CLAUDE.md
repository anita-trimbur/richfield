...
| `menu-icon-exit`                 | 48px                                                      | How far the menu button's icon travels as it leaves         |
...
| `card-link-bump`                 | 6px                                                       | A card icon's click bump, and the room kept for it          |
| `animate-card-link-bump`         | `card-link-bump` 260ms, out and back                      | A card icon's bump when the card is clicked                 |
...
`SiteHeader` (`src/components/site-header/`) is one header in two layouts. `useNavMode` measures the brand lockup, nav and links against the row's width and picks `"desktop"` (all three in a row) or `"mobile"` (nav and links replaced by the menu button); the desktop areas stay rendered but invisible and out of flow so they can keep being measured. The menu closes itself on a route change or a mode switch, and Escape closes it and returns focus to the button.

`PrimaryNavList` renders the links in both layouts, so it takes the layout as `className` and hands `itemClassName`, `linkClassName` and `iconClassName` down to each item, link and icon — that's where the mobile menu's 24px type, icons and row fade come from. It also sets `--nav-order` on every item, its place in the list, for a caller that wants them in order. `useIconSlider` measures slots at runtime, so the icons slide the same way at either size.

The mobile panel is a full-bleed `limestone-100` band below the header row, sitting outside the header's `PageContainer` with its own inside, so its rows still line up with the lockup; rows are divided by `limestone-200` and the external links are left out of it (the footer has them). In desktop mode the panel is `display: none`, like the menu button; in mobile mode it stays in the page, collapsed to nothing and `inert` when closed, which is what lets it slide rather than appear: opening grows its single grid row (`menu-panel-open`) while the labels fade in one after another behind it (`menu-row-shown`, held back by each row's `--nav-order`), and closing pulls the band shut with the rows fading out together. Reduced motion gets the panel and its rows with no travel. The same open state adds the panel's bottom padding, the gap it keeps above the page content, which is why the band's `limestone-100` sits on the row's content and not on the panel. Two things keep the closed panel at exactly zero height: the collapsing grid item carries no padding of its own (a `0fr` row can't squeeze padding away), and `PageContainer` stays an ordinary block inside it, since a grid item's `mx-auto` shrinks it to its content instead of filling the width.

`MenuToggleIcon` is the button's glyph: a hamburger and an X stacked in one 24px slot. Opening slides the hamburger out past the screen edge and brings the X back in behind it, closing does the same in reverse, and the button's `aria-expanded` drives all of it in CSS (`menu-icon-in` / `menu-icon-out` in `globals.css`), with no state of its own.

### Home timeline

Edit sections and cards in `src/content/timeline.ts`. Cards are `"accent"` (tall, random accent fill) or `"plain"` (short, `limestone-100`). A full-URL `href` opens in a new tab with an external-link icon; a path gets an arrow. A section's `icon` is an icon component from `src/components/icons/`, drawn at 32px in its marker box (`TimelineMarker` sets that size for all of them); a section without one gets an empty box. `AccentShuffle` (`src/components/accents/`) gives each accent card a random fill with no repeats until all 10 are used: an inline script does it on a full page load, before paint, and a layout effect does it on client-side navigation. It deals the classes it's given to any `data-accent` element inside it, so the footer's brand lockup reuses it for a random text color. To add or remove an accent color, change the token in `globals.css` and both class lists in `src/lib/accents.ts` (fills for cards, text colors for footer tiles and the footer lockup; same order in each).

Clicking any card bumps its link icon (arrow or external-link) right and back, so the click is acknowledged while the page navigates. `CardLink` is the one client piece of a card: it flags the link `data-bumping` on click (including Enter on the focused link) and clears the flag when the animation ends. The distance and timing are the `card-link-bump` tokens, and the card title carries `pr-card-link-bump` so the icon always has that much room before the card's padding. The title's last word and the icon share a `whitespace-nowrap` span, so the icon never wraps onto a line alone.
...
`SiteFooter` (`src/components/site-footer/`) renders on every page from `layout.tsx`, on `ink` with `limestone-50` text, inside `PageContainer` so it lines up with the header. `mt-auto` in the body's flex column pushes it to the viewport bottom on pages whose content doesn't fill the screen. It reuses `BrandLockup` (`src/components/brand/`, shared with the header) and `ExternalLinkList` (shared with the header's `ExternalLinks`, which sets the header's link colors). Here the lockup's mark and name get a random accent text color per page load, from the timeline's `AccentShuffle`; the role subtitle stays `limestone-50`. `CurrentYear` is the one client piece of the text column: the year in the exported HTML is whenever the site was last built, and it corrects it in the browser.
